...
function analyzeCapturedFrame(source, { measurementMode='full_zone_diameter', plateDiameterMm=PETRI_DISH_DIAMETER_MM }={}) {
  ...
  const candidates=[]; const nodeRadius=Math.max(5,Math.round(edgeRadius*.028))
  for(let y=nodeRadius+3;y<height-nodeRadius-3;y+=3) for(let x=nodeRadius+3;x<width-nodeRadius-3;x+=3){
    const distance=Math.hypot(x-cx,y-cy); if(distance>edgeRadius*.9) continue
    const center=gray[y*width+x]; let inner=0,outer=0,outerSq=0,n=0
    for(let a=0;a<Math.PI*2;a+=Math.PI/12){ const ix=Math.round(x+Math.cos(a)*nodeRadius*.65),iy=Math.round(y+Math.sin(a)*nodeRadius*.65); const ox=Math.round(x+Math.cos(a)*nodeRadius*1.35),oy=Math.round(y+Math.sin(a)*nodeRadius*1.35); const iv=gray[Math.max(0,Math.min(height-1,iy))*width+Math.max(0,Math.min(width-1,ix))]; const ov=gray[Math.max(0,Math.min(height-1,oy))*width+Math.max(0,Math.min(width-1,ox))]; inner+=iv;outer+=ov;outerSq+=ov*ov;n++ }
    const innerMean=inner/n, outerMean=outer/n, outerVariance=outerSq/n-outerMean*outerMean, contrast=Math.abs(innerMean-outerMean), circularity=Math.max(0,1-Math.sqrt(Math.max(0,outerVariance))/35)
    if(contrast>10 && circularity>.35) candidates.push({x,y,contrast,circularity,distance})
  }
  candidates.sort((a,b)=>(b.contrast*b.circularity)-(a.contrast*a.circularity)); const discs=[]
  for(const c of candidates){ if(discs.some(d=>Math.hypot(d.x-c.x,d.y-c.y)<nodeRadius*2.5)) continue; discs.push(c); if(discs.length>=14) break }
  ...
  const measured=discs.map((d,index)=>{ let bestR=discPx*1.35; let bestScore=0; for(let r=discPx*1.35;r<Math.min(edgeRadius*.28,discPx*8);r+=2){ let ring=0,nextRing=0,n=0; for(let a=0;a<Math.PI*2;a+=Math.PI/24){ const x=Math.max(0,Math.min(width-1,Math.round(d.x+Math.cos(a)*r))),y=Math.max(0,Math.min(height-1,Math.round(d.y+Math.sin(a)*r))); const nx=Math.max(0,Math.min(width-1,Math.round(d.x+Math.cos(a)*(r+3)))),ny=Math.max(0,Math.min(height-1,Math.round(d.y+Math.sin(a)*(r+3)))); ring+=gray[y*width+x];nextRing+=gray[ny*width+nx];n++ } const boundary=Math.abs(nextRing/n-ring/n); if(boundary>bestScore){bestScore=boundary;bestR=r} } const diameterMm=(measurementMode==='clear_zone_outside_disc' ? Math.max(0,(bestR*2-discPx)/pixelsPerMm) : bestR*2/pixelsPerMm); const confidence=Math.max(.35,Math.min(.92,(d.contrast/70)*d.circularity)); const nodeType=d.distance>edgeRadius*.55?'outer':'inner'; return {id:index+1,antibiotic:'UNKNOWN',zoneDiameterMm:Number(diameterMm.toFixed(1)),zoneRadiusMm:Number((diameterMm/2).toFixed(1)),discRadiusPx:discPx/2,zoneRadiusPx:bestR,discCenter:[d.x,d.y],x:d.x/width*100,y:d.y/height*100,nodeType,confidence,reviewRequired:true,corrected:false} })
